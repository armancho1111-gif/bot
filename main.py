...
async def handle_photo(message: Message):
    ...
    add_user(uid)
    if not can_use(uid):
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("Купить доступ 💰", callback_data="buy_access"))
        await message.answer("❌ Лимит бесплатных задач исчерпан.", reply_markup=markup)
        return
    photo = message.photo[-1]
    file = await bot.get_file(photo.file_id)
    downloaded = await bot.download_file(file.file_path)
    with open("task.jpg", "wb") as f:
        f.write(downloaded.read())
    text = extract_text("task.jpg")
    await process_task(uid, text, message)
...
async def process_task(uid, task, message_obj):
    ...
    if not can_use(uid):
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("Купить доступ 💰", callback_data="buy_access"))
        await message_obj.answer("❌ Лимит бесплатных задач исчерпан.", reply_markup=markup)
        return
...
async def successful_payment(message: Message):
    if message.successful_payment:
        ...
        await message.answer("✅ Оплата получена! Безлимитный доступ активирован.", reply_markup=get_repeat_buttons())
